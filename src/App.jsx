...
import { Heart, Coins, Map as MapIcon, Sword, Shield, Skull, Ghost, Sparkles, ScrollText, Library, Crosshair, Hexagon, Eye, Flame, Wind, Loader2, Globe } from 'lucide-react';
import { UI as LOCALIZATION_UI, CHARACTER_TEXT, RELIC_POOL as LOCALIZATION_RELIC_POOL, RELIC_DICT as LOCALIZATION_RELIC_DICT, CARD_DICT as LOCALIZATION_CARD_DICT, ENEMY_LIBRARY as LOCALIZATION_ENEMY_LIBRARY, ENEMY_MOVE_LABELS as LOCALIZATION_ENEMY_MOVE_LABELS, ACHIEVEMENT_DEFS as LOCALIZATION_ACHIEVEMENT_DEFS, COPY, localizeTemplate, localizeGeneratedName, translateGeneratedText } from './localization';
...
const renderTextWithIcons = (text) => {
  ...
  return parts.map((part, i) => {
    if (part === 'damage' || part === 'Dmg') {
      return <span key={i} className="inline-flex items-center text-red-400 font-bold mx-0.5 drop-shadow-sm"><Sword className="w-3.5 h-3.5 mr-0.5" />{part}</span>;
    }
    if (part === 'Block') {
...
const CHARACTERS = {
  ...
  WATCHER: { 
    ...
    cardPool: ["Flurry of Blows", "Empty Fist", "Halt", "Bowling Bash", "Tantrum", "Wallop", "Vault", "Blasphemy", "Sash Whip", "Cut Through Fate"]
  },
  NECROBINDER: { 
    ...
    cardPool: ["Soul Strike", "Bone Wall", "Dark Bargain", "Grave Dig", "Summon Horde", "Death Knell", "Siphon", "Corpse Explosion", "Ritual Dagger", "Spectral Shield"]
  }
};

const RELIC_POOL = LOCALIZATION_RELIC_POOL;
const RELIC_DICT = LOCALIZATION_RELIC_DICT;
const CARD_DICT = LOCALIZATION_CARD_DICT;
...
const getStateRewardRelicPool = (state) => {
  const unlockedPool = RELIC_POOL.filter(relic => (state.unlockedRelics || []).includes(relic));
  return unlockedPool.length > 0 ? unlockedPool : RELIC_POOL;
};
...
const calcDamage = (base, combat) => {
  let dmg = base + combat.playerStrength;
...
const executeCard = (cardName, s, c) => {
  if (cardName === 'Wound') return; 
  
  const isUpgraded = cardName.endsWith('+');
  const baseName = cardName.replace('+', '');
  let mult = isUpgraded ? 1.5 : 1;

  if (s.relics.includes('Pen Nib')) {
      c.cardsPlayed = (c.cardsPlayed || 0) + 1;
      if (c.cardsPlayed % 3 === 0) mult *= 2;
  }

  if (s.relics.includes('Cracked Core')) c.playerBlock += 2;

  const dmg = (val) => applyDamageToEnemy(calcDamage(Math.floor(val * mult), c), c);
  const blk = (val) => c.playerBlock += Math.floor(val * mult);
  const heal = (val) => s.hp = Math.min(s.maxHp, s.hp + Math.floor(val * mult));
  const str = (val) => c.playerStrength += Math.floor(val * mult);
  const vuln = (val) => c.enemyVuln += Math.floor(val * mult);
  const weak = (val) => c.enemyWeak += Math.floor(val * mult);

  switch(baseName) {
    case 'Strike': dmg(12); break;
    case 'Defend': blk(10); break;
    case 'Bash': dmg(16); vuln(2); break;
    case 'Iron Wave': blk(10); dmg(10); break;
    case 'Perfected Strike': dmg(12 + (s.deck.filter(card => card.includes('Strike')).length * 4)); break;
    case 'Pommel Strike': dmg(20); break;
    case 'Shrug It Off': blk(16); heal(2); break;
    case 'Cleave': dmg(16); break;
    case 'Inflame': str(3); break;
    case 'Shockwave': weak(3); vuln(3); break;
    case 'Bludgeon': dmg(45); break;
    case 'Demon Form': str(5); break;
    case 'Limit Break': c.playerStrength *= 2; break;
    case 'Carnage': dmg(35); break;
    case 'Survivor': blk(15); break;
    case 'Neutralize': dmg(8); weak(2); break;
    case 'Zap': dmg(12); break;
    case 'Dualcast': dmg(24); break;
    case 'Eruption': dmg(18); vuln(2); break;
    case 'Vigilance': blk(16); break;
    case 'Reap': dmg(12); heal(4); break;
    case 'Animate': dmg(10); blk(10); break;

    case 'Dagger Spray': dmg(8); break;
    case 'Backflip': blk(10); drawCards(c, Math.floor(2 * mult)); break;
    case 'Deadly Poison': dmg(10); weak(2); break;
    case 'Acrobatics': drawCards(c, Math.floor(3 * mult)); break;
    case 'Bouncing Flask': dmg(15); break;
    case 'Adrenaline': str(1); drawCards(c, Math.floor(2 * mult)); break;
    case 'Catalyst': vuln(5); break;
    case 'Noxious Fumes': dmg(5); break;
    case 'Dash': blk(10); dmg(10); break;
    case 'Cloak And Dagger': blk(6); dmg(6); break;

    case 'Cold Snap': dmg(12); blk(5); break;
    case 'Ball Lightning': dmg(14); break;
    case 'Glacier': blk(15); break;
    case 'Hologram': blk(10); drawCards(c, Math.floor(1 * mult)); break;
    case 'Defragment': str(2); break;
    case 'Echo Form': str(5); break;
    case 'Leap': blk(18); break;
    case 'Sweeping Beam': dmg(12); break;
    case 'Claw': 
        c.clawBase = c.clawBase || 6; 
        dmg(c.clawBase); 
        c.clawBase += 4; 
        break;
    case 'Core Surge': dmg(22); break;

    case 'Flurry of Blows': dmg(8); break;
    case 'Empty Fist': dmg(18); break;
    case 'Halt': blk(14); break;
    case 'Bowling Bash': dmg(15); break;
    case 'Tantrum': dmg(20); break;
    case 'Wallop': dmg(18); blk(18); break;
    case 'Vault': drawCards(c, Math.floor(3 * mult)); break;
    case 'Blasphemy': c.playerStrength *= 2; break;
    case 'Sash Whip': dmg(16); weak(2); break;
    case 'Cut Through Fate': dmg(14); drawCards(c, Math.floor(1 * mult)); break;

    case 'Soul Strike': dmg(15); heal(2); break;
    case 'Bone Wall': blk(18); break;
    case 'Dark Bargain': s.hp = Math.max(1, s.hp - 5); str(3); break;
    case 'Grave Dig': drawCards(c, Math.floor(2 * mult)); break;
    case 'Summon Horde': dmg(20); blk(10); break;
    case 'Death Knell': dmg(35); break;
    case 'Siphon': dmg(12); heal(8); break;
    case 'Corpse Explosion': dmg(25); break;
    case 'Ritual Dagger': dmg(20); break;
    case 'Spectral Shield': blk(20); break;
  }
};
...
const generateProceduralRun = (act) => {
  ...
  const generateEvent = () => {
    ...
    return {
      ...
      choices: [
        { label: COPY.nodes.mysteriousDiscovery.takeRisk, effectText: localizeTemplate(COPY.nodes.mysteriousDiscovery.takeRiskEffect, { hpCost }), action: (state) => ({ hp: state.hp - hpCost, relics: [...state.relics, getRandomItem(getStateRewardRelicPool(state))] }) },
...
const getRewardNode = (enemyId) => ({
  ...
  choices: [
    { label: COPY.nodes.reward.takeGold, effectText: COPY.nodes.reward.takeGoldEffect, action: (state) => ({ gold: state.gold + 25 }) },
    { label: COPY.nodes.reward.addCard, effectText: COPY.nodes.reward.addCardEffect, action: (state) => ({ deck: [...state.deck, getRandomItem(getStateRewardCardPool(state))] }) },
    { label: COPY.nodes.reward.bandage, effectText: COPY.nodes.reward.bandageEffect, action: (state) => ({ hp: Math.min(state.maxHp, state.hp + 15) }) }
  ]
});
...
const getNeowNode = () => ({
  ...
  choices: [
    ...
    { label: COPY.nodes.neow.sacrifice, effectText: COPY.nodes.neow.sacrificeEffect, action: (state) => ({ maxHp: state.maxHp - 8, hp: Math.min(state.hp, state.maxHp - 8), relics: [...state.relics, getRandomItem(getStateRewardRelicPool(state))] }) },
...
const getRestSite = () => {
  const choices = [
    ...
    { label: COPY.nodes.rest.dig, effectText: COPY.nodes.rest.digEffect, condition: (state) => state.relics.includes('Shovel'), action: (state) => ({ relics: [...state.relics, getRandomItem(getStateRewardRelicPool(state))] }) }
  ];
  return {
...
const getShop = () => ({
  ...
  choices: [
    { label: COPY.nodes.shop.buyRelic, effectText: COPY.nodes.shop.buyRelicEffect, condition: (state) => state.gold >= 75, action: (state) => ({ gold: state.gold - 75, relics: [...state.relics, getRandomItem(getStateRewardRelicPool(state))] }) },
...
const generateMapRoute = (runContent) => {
  ...
  const createCombatNode = (enemy) => ({
    title: enemy.names, type: "Combat", spriteKey: enemy.spriteKey, icon: <Sword className="w-12 h-12 text-red-500" />,
...
export default function App() {
  ...
  const bgmLoopRef = React.useRef(null);
  const bgmNodesRef = React.useRef([]);
  const bgmBarRef = React.useRef(0);
  ...
  const playSound = useCallback((type = 'click') => {
    try {
      ...
      const volumeScale = Math.min(4.2, Math.max(0, masterVolume * channelVolume * 2.9));
  ...
  useEffect(() => {
    ...
    const settings = metaProgress.settings || createDefaultMetaProgress().settings;
    const roomChannel = ['Combat', 'Elite', 'Boss'].includes(currentRoomType) ? settings.combatVolume : settings.uiVolume;
    const bgmVolume = Math.max(0, Math.min(1, ((settings.masterVolume ?? 0) / 100) * ((roomChannel ?? 0) / 100) * 0.8));
    if (bgmVolume <= 0) return undefined;
    ...
    const themes = {
      Combat: {
        ...
        padGain: 0.06,
        bassBars: [
          [110, 123.47, 146.83, 123.47, 98, 123.47, 146.83, 164.81],
          [110, 130.81, 146.83, 164.81, 110, 98, 123.47, 146.83]
        ],
        leadBars: [
          [220, 246.94, 293.66, 329.63, 293.66, 349.23, 392, 349.23],
          [220, 261.63, 293.66, 349.23, 392, 440, 392, 329.63]
        ],
        pads: [[220, 293.66, 349.23], [196, 246.94, 329.63]]
      },
      Elite: {
        step: 0.34,
        wave: 'triangle',
        ...
        padGain: 0.055,
        bassBars: [
          [98, 116.54, 130.81, 155.56, 130.81, 116.54, 155.56, 174.61],
          [98, 123.47, 146.83, 155.56, 174.61, 155.56, 146.83, 130.81]
        ],
        leadBars: [
          [196, 233.08, 311.13, 293.66, 349.23, 392, 349.23, 311.13],
          [233.08, 261.63, 311.13, 369.99, 415.3, 369.99, 311.13, 261.63]
        ],
        pads: [[196, 261.63, 311.13], [233.08, 293.66, 369.99]]
      },
      Boss: {
        step: 0.38,
        wave: 'sawtooth',
        ...
        padGain: 0.07,
        bassBars: [
          [82.41, 98, 123.47, 155.56, 164.81, 155.56, 123.47, 98],
          [82.41, 103.83, 123.47, 164.81, 174.61, 155.56, 123.47, 110]
        ],
        leadBars: [
          [164.81, 196, 246.94, 311.13, 369.99, 415.3, 369.99, 311.13],
          [196, 246.94, 293.66, 369.99, 415.3, 493.88, 415.3, 311.13]
        ],
        pads: [[164.81, 246.94, 311.13], [196, 293.66, 369.99]]
      },
      Event: {
        step: 0.36,
        wave: 'sine',
        ...
        padGain: 0.07,
        bassBars: [
          [130.81, 146.83, 164.81, 196, 164.81, 146.83, 196, 220],
          [130.81, 164.81, 174.61, 196, 220, 196, 174.61, 164.81]
        ],
        leadBars: [
          [392, 440, 493.88, 587.33, 659.25, 587.33, 523.25, 440],
          [440, 493.88, 523.25, 659.25, 739.99, 659.25, 587.33, 493.88]
        ],
        pads: [[392, 493.88, 587.33], [440, 523.25, 659.25]]
      },
      Rest: {
        step: 0.44,
        wave: 'sine',
        ...
        padGain: 0.09,
        bassBars: [
          [130.81, 146.83, 164.81, 146.83, 130.81, 146.83, 174.61, 196],
          [130.81, 164.81, 174.61, 196, 174.61, 164.81, 146.83, 130.81]
        ],
        leadBars: [
          [392, 440, 493.88, 587.33, 659.25, 739.99, 659.25, 587.33],
          [440, 493.88, 587.33, 659.25, 783.99, 880, 783.99, 659.25]
        ],
        pads: [[392, 493.88, 659.25], [440, 587.33, 783.99]]
      },
      Shop: {
        step: 0.28,
        wave: 'square',
        ...
        padGain: 0.05,
        bassBars: [
          [164.81, 196, 220, 196, 164.81, 196, 246.94, 261.63],
          [174.61, 196, 220, 261.63, 246.94, 220, 196, 174.61]
        ],
        leadBars: [
          [523.25, 659.25, 783.99, 659.25, 880, 987.77, 880, 783.99],
          [659.25, 783.99, 880, 987.77, 1174.66, 987.77, 880, 783.99]
        ],
        pads: [[523.25, 659.25, 783.99], [587.33, 783.99, 987.77]]
      },
      Victory: {
        step: 0.4,
        wave: 'triangle',
        ...
        padGain: 0.08,
        bassBars: [
          [196, 246.94, 293.66, 392, 440, 392, 293.66, 246.94],
          [220, 261.63, 329.63, 392, 523.25, 440, 392, 329.63]
        ],
        leadBars: [
          [523.25, 659.25, 783.99, 1046.5, 1174.66, 1318.5, 1174.66, 1046.5],
          [659.25, 783.99, 1046.5, 1174.66, 1318.5, 1567.98, 1318.5, 1046.5]
        ],
        pads: [[523.25, 659.25, 783.99], [659.25, 783.99, 1046.5]]
      },
      'Game Over': {
        step: 0.46,
        wave: 'triangle',
        ...
        padGain: 0.05,
        bassBars: [
          [82.41, 77.78, 73.42, 69.3, 65.41, 69.3, 73.42, 77.78],
          [82.41, 73.42, 69.3, 65.41, 61.74, 65.41, 69.3, 73.42]
        ],
        leadBars: [
          [164.81, 155.56, 146.83, 130.81, 123.47, 130.81, 146.83, 155.56],
          [155.56, 146.83, 130.81, 123.47, 110, 123.47, 130.81, 146.83]
        ],
        pads: [[130.81, 164.81, 196], [123.47, 155.56, 185]]
      },
      Transition: {
        step: 0.32,
        wave: 'triangle',
        ...
        padGain: 0.05,
        bassBars: [
          [146.83, 164.81, 196, 246.94, 293.66, 246.94, 196, 164.81],
          [146.83, 174.61, 220, 246.94, 329.63, 293.66, 246.94, 196]
        ],
        leadBars: [
          [293.66, 392, 440, 523.25, 587.33, 523.25, 440, 392],
          [329.63, 392, 493.88, 587.33, 659.25, 587.33, 493.88, 392]
        ],
        pads: [[293.66, 392, 523.25], [329.63, 440, 587.33]]
      }
    };

    const theme = themes[currentRoomType] || themes.Event;
    const isBattleTheme = ['Combat', 'Elite', 'Boss'].includes(currentRoomType);
    const roomPhraseStyle = {
      Combat: { lift: 3, shadow: -2, restEvery: 4, accentWave: 'square', accentGain: 0.06 },
      Elite: { lift: 4, shadow: -3, restEvery: 4, accentWave: 'triangle', accentGain: 0.065 },
      Boss: { lift: 5, shadow: -5, restEvery: 0, accentWave: 'sawtooth', accentGain: 0.075 },
      Event: { lift: 7, shadow: 2, restEvery: 3, accentWave: 'sine', accentGain: 0.045 },
      Rest: { lift: 5, shadow: 0, restEvery: 4, accentWave: 'sine', accentGain: 0.04 },
      Shop: { lift: 7, shadow: 2, restEvery: 3, accentWave: 'triangle', accentGain: 0.05 },
      Victory: { lift: 7, shadow: 4, restEvery: 0, accentWave: 'triangle', accentGain: 0.055 },
      'Game Over': { lift: -2, shadow: -5, restEvery: 5, accentWave: 'sine', accentGain: 0.03 },
      Transition: { lift: 5, shadow: 0, restEvery: 4, accentWave: 'triangle', accentGain: 0.04 }
    }[currentRoomType] || { lift: 5, shadow: 0, restEvery: 4, accentWave: 'triangle', accentGain: 0.04 };
    const transpose = (freq, semitones = 0) => (!freq ? 0 : Number((freq * Math.pow(2, semitones / 12)).toFixed(2)));
    const extendBars = (bars, options = {}) => {
      const { lift = 0, shadow = 0, restEvery = 0 } = options;
      const lifted = bars.map((bar, barIndex) => bar.map((freq, noteIndex) => {
        if (!freq) return 0;
        if (restEvery && noteIndex % restEvery === restEvery - 1) return 0;
        const semitoneShift = noteIndex % 2 === 0 ? lift : Math.max(0, lift - 2);
        return transpose(freq, semitoneShift + (barIndex % 2 === 0 ? 0 : 12));
      }));
      const shadowed = bars.map((bar, barIndex) => bar.map((freq, noteIndex) => {
        if (!freq) return 0;
        if (noteIndex === 0 && barIndex % 2 === 1) return 0;
        return transpose(freq, noteIndex % 3 === 0 ? shadow : shadow + 2);
      }));
      return [...bars, ...lifted, ...shadowed].slice(0, Math.max(4, bars.length * 2));
    };
    const bassBars = extendBars(theme.bassBars, { lift: Math.max(0, roomPhraseStyle.lift - 5), shadow: -5, restEvery: 0 });
    const leadBars = extendBars(theme.leadBars, roomPhraseStyle);
    const pads = extendBars(theme.pads, { lift: Math.max(0, roomPhraseStyle.lift - 7), shadow: roomPhraseStyle.shadow }).map(bar => bar.filter(Boolean).slice(0, 3));
    bgmBarRef.current = 0;
    const playBar = () => {
      const barIndex = bgmBarRef.current % bassBars.length;
      const bassBar = bassBars[barIndex];
      const leadBar = leadBars[barIndex % leadBars.length];
      const pad = pads[barIndex % pads.length];
      const barStart = ctx.currentTime + 0.03;
      const bassPeak = Math.max(0.0001, bgmVolume * (theme.bassGain || 0.18));
      const leadPeak = Math.max(0.0001, bgmVolume * (theme.leadGain || 0.13));
      const padPeak = Math.max(0.0001, bgmVolume * (theme.padGain || 0.06));

      bassBar.forEach((freq, index) => {
        if (!freq) return;
        const start = barStart + index * theme.step;
        const osc = ctx.createOscillator();
        const gain = ctx.createGain();
        const filter = ctx.createBiquadFilter();
        osc.type = theme.bassWave || 'triangle';
        osc.frequency.setValueAtTime(freq, start);
        if (isBattleTheme && index % 2 === 0) {
          osc.detune.setValueAtTime(index % 4 === 0 ? -6 : 6, start);
        }
        filter.type = 'lowpass';
        filter.frequency.setValueAtTime(theme.bassCutoff || (isBattleTheme ? 520 : 780), start);
        gain.gain.setValueAtTime(0.0001, start);
        gain.gain.exponentialRampToValueAtTime(bassPeak, start + 0.05);
        gain.gain.exponentialRampToValueAtTime(0.0001, start + theme.step * 1.75);
        osc.connect(filter);
        filter.connect(gain);
        gain.connect(ctx.destination);
        osc.start(start);
        osc.stop(start + theme.step * 1.8);
        bgmNodesRef.current.push(osc, gain, filter);
      });

      leadBar.forEach((freq, index) => {
        if (!freq) return;
        const start = barStart + index * theme.step + theme.step * 0.35;
        const osc = ctx.createOscillator();
        const gain = ctx.createGain();
        osc.type = theme.wave;
        osc.frequency.setValueAtTime(freq, start);
        if (!isBattleTheme && index % 3 === 0) {
          osc.detune.setValueAtTime(5, start);
        }
        gain.gain.setValueAtTime(0.0001, start);
        gain.gain.exponentialRampToValueAtTime(leadPeak, start + 0.04);
        gain.gain.exponentialRampToValueAtTime(0.0001, start + theme.step * 1.2);
        osc.connect(gain);
        gain.connect(ctx.destination);
        osc.start(start);
        osc.stop(start + theme.step * 1.25);
        bgmNodesRef.current.push(osc, gain);
      });

      pad.forEach((freq, index) => {
        if (!freq) return;
        const start = barStart + index * theme.step * 2;
        const osc = ctx.createOscillator();
        const gain = ctx.createGain();
        const filter = ctx.createBiquadFilter();
        osc.type = theme.padWave || 'sine';
        osc.frequency.setValueAtTime(freq, start);
        if (!isBattleTheme) {
          osc.detune.setValueAtTime(index % 2 === 0 ? 8 : -8, start);
        }
        filter.type = 'lowpass';
        filter.frequency.setValueAtTime(Math.max(220, (theme.bassCutoff || (isBattleTheme ? 520 : 780)) + (isBattleTheme ? -80 : 120)), start);
        gain.gain.setValueAtTime(0.0001, start);
        gain.gain.exponentialRampToValueAtTime(padPeak, start + 0.08);
        gain.gain.exponentialRampToValueAtTime(0.0001, start + theme.step * 3.6);
        osc.connect(filter);
        filter.connect(gain);
        gain.connect(ctx.destination);
        osc.start(start);
        osc.stop(start + theme.step * 3.7);
        bgmNodesRef.current.push(osc, gain, filter);
      });

      leadBar.forEach((freq, index) => {
        if (!freq || index % 2 === 0) return;
        const start = barStart + index * theme.step + theme.step * 0.12;
        const osc = ctx.createOscillator();
        const gain = ctx.createGain();
        const filter = ctx.createBiquadFilter();
        osc.type = roomPhraseStyle.accentWave;
        osc.frequency.setValueAtTime(transpose(freq, isBattleTheme ? -12 : 12), start);
        filter.type = 'lowpass';
        filter.frequency.setValueAtTime(isBattleTheme ? 1400 : 2200, start);
        gain.gain.setValueAtTime(0.0001, start);
        gain.gain.exponentialRampToValueAtTime(Math.max(0.0001, bgmVolume * roomPhraseStyle.accentGain), start + 0.03);
        gain.gain.exponentialRampToValueAtTime(0.0001, start + theme.step * 0.9);
        osc.connect(filter);
        filter.connect(gain);
        gain.connect(ctx.destination);
        osc.start(start);
        osc.stop(start + theme.step);
        bgmNodesRef.current.push(osc, gain, filter);
      });
    ...
    playBar();
    bgmLoopRef.current = window.setInterval(playBar, theme.step * 8 * 1000);
  ...
  }, [currentRoomType, getAudioContext, metaProgress.settings]);
  ...
  const addRewardEffect = useCallback((type, text) => {
    const newEffect = {
  ...
  const handleChoice = useCallback((choice) => {
    ...
    if (result.gold !== undefined && result.gold > gameState.gold) { addRewardEffect('gold', `+${result.gold - gameState.gold}`); playSound('coin'); }
    if (result.gold !== undefined && result.gold < gameState.gold) addRewardEffect('gold', `${result.gold - gameState.gold}`);
  ...
  const playCardInCombat = async (cardName, cardIndex) => {
    ...
    let c = { ...s.combat, hand: [...s.combat.hand], drawPile: [...s.combat.drawPile], discardPile: [...s.combat.discardPile] };
    s.combat = c;
    ...
    if (dealtDamage) {
        setEnemyAnim('hurt');
        addRewardEffect('damage', `-${preEnemyHp - c.enemyHp}`);
    }
    ...
    if (c.enemyHp <= 0) {
      ...
      s.combat = null;
      setGameState({...s});
      setCurrentNode(getRewardNode(c.enemyId));
      recordMetaProgress({
        totalCombatWins: 1,
    ...
    // Upkeep Phase
    if (s.relics.includes('Orichalcum') && c.playerBlock === 0) { c.playerBlock += 6; addRewardEffect('heal', '+6 Block'); playSound('block'); }
    c.playerBlock = 0;
  ...
  const renderRewardIcon = (type) => {
     if(type==='gold') return <Coins className="w-6 h-6"/>;
     if(type==='relic') return <Sparkles className="w-6 h-6"/>;
     if(type==='card') return <ScrollText className="w-6 h-6"/>;
     if(type==='heal') return <Heart className="w-6 h-6"/>;
     if(type==='damage') return <Sword className="w-6 h-6"/>;
     return null;
  };

  const renderAchievementIcon = (iconKey, className = "w-6 h-6") => {
    if (iconKey === 'sword') return <Sword className={className} />;
    if (iconKey === 'coins') return <Coins className={className} />;
    if (iconKey === 'sparkles') return <Sparkles className={className} />;
    if (iconKey === 'scroll') return <ScrollText className={className} />;
  ...
  if (!gameState.character) {
    return (
      <div className="min-h-screen bg-black flex flex-col items-center justify-center p-4 md:p-6 text-slate-200 selection:bg-red-900 selection:text-white relative overflow-x-hidden overflow-y-auto">
        ...
        {/* Language Switcher */}
        <div className="absolute top-4 right-4 z-50 flex items-center gap-2 bg-slate-900/80 p-2 rounded border border-slate-700">
            <Globe className="w-4 h-4 text-slate-400" />
            <select value={lang} onChange={handleLangChange} className="bg-transparent text-sm font-bold outline-none cursor-pointer">
                ...
                return (
                  ...
                  >
                    ...
                    <div className="text-center w-full">
                      ...
                      <p className="text-sm text-slate-400 mb-2 flex justify-between px-2"><span>{UI.relics[lang]}:</span> <span className="text-slate-200 font-bold text-right ml-2">{getTranslatedRelic(char.relics[0], lang).name}</span></p>
                            ...
                            return (
                              <div key={relic} className={`rounded-xl border p-3 ${isUnlocked ? 'bg-slate-800 border-slate-600 text-slate-200' : 'bg-slate-950 border-slate-800 text-slate-500'}`}>
                                <div className="flex items-center justify-between gap-3">
                                  <span className="font-bold">{translatedRelic.name}</span>
                      ...
                      <div>
                        ...
                        <div className="overflow-x-auto rounded-xl border border-slate-700">
                          <table className="w-full text-left text-sm">
                            <thead className="bg-slate-950/90 text-slate-400 uppercase tracking-widest text-xs">
                              <tr>
                                <th className="px-4 py-3">{UI.deck[lang]}</th>
                                <th className="px-4 py-3">{UI.status[lang]}</th>
                                <th className="px-4 py-3">{UI.effect[lang]}</th>
                              </tr>
                            </thead>
                            <tbody>
                              {cards.map((card, index) => {
                                ...
                                return (
                                  <tr key={card} className={index % 2 === 0 ? 'bg-slate-900' : 'bg-slate-950/90'}>
                                    <td className={`px-4 py-3 font-bold ${isUnlocked ? 'text-slate-100' : 'text-slate-500'}`}>{translatedCard.name}</td>
                                    <td className="px-4 py-3">
                                      <span className={`text-[10px] uppercase tracking-widest ${isUnlocked ? 'text-emerald-400' : 'text-amber-400'}`}>{isUnlocked ? UI.unlocked[lang] : UI.locked[lang]}</span>
                                    </td>
                                    <td className={`px-4 py-3 ${isUnlocked ? 'text-slate-300' : 'text-slate-500'}`}>{translatedCard.desc}</td>
                                  </tr>
                                );
                              })}
                            </tbody>
                    ...
                    <div className="bg-slate-900/90 border border-slate-700 rounded-2xl p-5">
                      ...
                      <div className="flex flex-wrap gap-2">
                        {generalRelics.map((relic) => (
                          <div key={relic} className={`px-2 py-1 rounded border text-xs ${metaProgress.unlockedRelics.includes(relic) ? 'bg-slate-800 border-slate-600 text-slate-200' : 'bg-slate-950 border-slate-800 text-slate-600'}`}>
  ...
  const charKeyString = charKeyObj.name.en.replace(/THE /i, '').trim().toLowerCase();
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-200 font-sans flex flex-col md:flex-row selection:bg-red-900 selection:text-white overflow-x-hidden overflow-y-auto md:overflow-hidden">
      ...
      {/* Language Switcher overlay for active game */}
      <div className="absolute top-3 right-3 md:top-4 md:right-4 z-[300] flex items-center gap-2 bg-slate-900/80 p-2 rounded border border-slate-700">
          <Globe className="w-4 h-4 text-slate-400" />
          <select value={lang} onChange={handleLangChange} className="bg-transparent text-sm font-bold outline-none cursor-pointer">
              <option value="en">English</option>
              <option value="ja">Japanese</option>
      ...
      <div className="relative w-full md:w-80 bg-slate-900 border-b md:border-b-0 md:border-r border-slate-700 p-4 md:p-6 flex flex-col gap-4 md:gap-6 shadow-2xl z-[150] max-h-[40vh] md:max-h-none overflow-y-auto">
        ...
        <div className="flex-1 min-h-0">
          <h2 className="text-sm font-semibold text-slate-400 uppercase tracking-wider mb-3 flex items-center gap-2">
            <Sparkles className="w-4 h-4" /> {UI.relics[lang]} ({gameState.relics.length})
          </h2>
          <div className="flex flex-wrap gap-2">
            {gameState.relics.map((relic, i) => {
              const tr = getTranslatedRelic(relic, lang);
              return (
              <div key={i} onMouseEnter={() => setHoveredInfo({ title: tr.name, desc: tr.desc || COPY.fallback.unknownEffect[lang] })} onMouseLeave={() => setHoveredInfo(null)} className="bg-slate-800 text-xs px-2 py-1 rounded border border-slate-600 text-slate-300 cursor-help hover:bg-slate-700 hover:-translate-y-0.5 transition-all shadow-sm">
                {tr.name}
              </div>
            )})}
          </div>
        ...
        <div className="flex-1 min-h-0 max-h-40 md:max-h-48 overflow-y-auto pr-2 custom-scrollbar">
          <h2 className="text-sm font-semibold text-slate-400 uppercase tracking-wider mb-3 flex items-center gap-2">
            <ScrollText className="w-4 h-4" /> {UI.deck[lang]} ({gameState.deck.length})
          </h2>
        ...
        <div className="flex-1 min-h-0 max-h-36 md:max-h-48 overflow-y-auto pr-2 custom-scrollbar bg-[#2a2420] rounded-lg border border-[#4a3b32] p-3 shadow-inner relative">
          ...
          <div className="flex flex-col gap-1 relative pl-2 z-10">
            ...
            {gameState.route.map((node, i) => {
              ...
              let icon;
              if (node.type === 'Combat') icon = <Sword className="w-3 h-3 text-[#d1b098]" />;
      ...
      <div className={`flex-1 relative flex flex-col items-center justify-start p-0 bg-black transition-colors duration-300 min-h-[60vh] md:min-h-screen ${effect === 'damage' ? 'bg-red-950/40' : ''}`}>
        ...
        {/* MAIN UI CARD */}
        <div className={`relative z-20 w-[calc(100%-1rem)] max-w-2xl mt-[17vh] md:mt-[26vh] mb-0 md:mb-4 bg-slate-900/90 backdrop-blur-md border border-slate-700/80 rounded-t-2xl md:rounded-2xl shadow-[0_0_50px_rgba(0,0,0,0.8)] overflow-hidden flex flex-col flex-1 max-h-[74vh] md:max-h-[85vh] transition-all ${isMazeMoving ? 'opacity-0 translate-y-24 scale-95 pointer-events-none duration-500' : 'animate-room-enter'} ${effect === 'damage' ? 'animate-shake border-red-500/70 shadow-red-900/50' : ''} ${effect === 'heal' ? 'border-green-500/70 shadow-green-900/50' : ''} ${effect === 'gain' ? 'border-yellow-500/70 shadow-yellow-900/50' : ''}`}>
          ...
          {gameState.combat?.active ? (
            ...
            /* --- COMBAT ARENA UI --- */
            <div className="flex flex-col w-full h-full relative z-10 overflow-y-auto custom-scrollbar pb-8">
              
              <div className="flex flex-col items-center justify-center relative p-6 border-b border-slate-700">
                <h3 className="text-2xl font-bold text-slate-100">{gameState.combat.enemyName[lang] || gameState.combat.enemyName.en}</h3>
                
                <div className="relative w-64 h-6 bg-slate-950 rounded-full border border-slate-700 overflow-hidden my-3 flex items-center justify-center shadow-inner">
                   <div className="absolute left-0 top-0 bottom-0 bg-red-600 transition-all duration-500" style={{ width: `${(gameState.combat.enemyHp / gameState.combat.enemyMaxHp) * 100}%`}}></div>
                   <span className="relative z-10 text-xs font-bold text-white tracking-widest">{gameState.combat.enemyHp} / {gameState.combat.enemyMaxHp}</span>
                </div>
                
                {gameState.combat.enemyBlock > 0 && (
                  <div className="text-blue-400 font-bold mb-2 flex items-center gap-1 bg-blue-900/20 px-3 py-1 rounded-full border border-blue-500/30"><Shield className="w-4 h-4"/> {UI.block[lang]}: {gameState.combat.enemyBlock}</div>
                )}
                
                <div className="text-yellow-400 animate-pulse text-sm mt-2 font-mono bg-yellow-900/20 px-4 py-1.5 rounded-full border border-yellow-700/50 flex items-center gap-2">
                   <Crosshair className="w-4 h-4" /> {UI.intent[lang]}: {renderTextWithIcons(gameState.combat.intent.text[lang] || gameState.combat.intent.text.en)}
                </div>
                
                <div className="flex gap-2 mt-4">
                   {gameState.combat.enemyStrength > 0 && <span className="text-orange-400 text-xs px-2 py-1 bg-orange-950/50 rounded border border-orange-900 drop-shadow">+{gameState.combat.enemyStrength} {UI.str[lang]}</span>}
                   {gameState.combat.enemyVuln > 0 && <span className="text-purple-400 text-xs px-2 py-1 bg-purple-950/50 rounded border border-purple-900 drop-shadow">{gameState.combat.enemyVuln} {UI.vuln[lang]}</span>}
                   {gameState.combat.enemyWeak > 0 && <span className="text-blue-400 text-xs px-2 py-1 bg-blue-950/50 rounded border border-blue-900 drop-shadow">{gameState.combat.enemyWeak} {UI.weak[lang]}</span>}
                </div>
              </div>

              <div className="bg-slate-800/80 p-4 flex justify-between items-center shadow-[0_-5px_20px_rgba(0,0,0,0.3)]">
                <div>
                   <div className="font-bold text-slate-300 text-xs uppercase tracking-widest mb-1">{UI.combat_status[lang]}</div>
                   <div className="text-sm flex gap-4">
                       <span className="text-blue-400 font-bold flex items-center gap-1"><Shield className="w-4 h-4"/> {UI.block[lang]}: {gameState.combat.playerBlock}</span>
                       {gameState.combat.playerStrength > 0 && <span className="text-orange-400 font-bold">{UI.str[lang]}: +{gameState.combat.playerStrength}</span>}
                       {gameState.combat.playerVuln > 0 && <span className="text-purple-400 font-bold">{UI.vuln[lang]}: {gameState.combat.playerVuln}</span>}
                   </div>
                </div>
                <div className="text-slate-500 text-xs uppercase tracking-widest text-right">
        ...
        {gameState.combat?.active && (
          <div className="fixed bottom-2 sm:bottom-3 md:bottom-6 left-2 right-20 sm:right-28 md:left-auto md:right-40 w-auto md:w-[34rem] flex justify-end items-end h-44 sm:h-52 md:h-64 z-[200] pointer-events-none" style={{ perspective: '1000px' }}>
            {gameState.combat.hand.map((card, idx) => {
               const tc = getTranslatedCard(card, lang);
               const isAttack = tc.desc && (tc.desc.includes('damage') || tc.desc.includes('Dmg'));
               
               const isPlaying = playedCardIndex === idx;
               const isHovered = hoveredCardIndex === idx && !isCombatResolving && !isPlaying;
               
               const bgClass = isAttack ? 'bg-gradient-to-b from-red-950/90 to-slate-900/90' : 'bg-gradient-to-b from-blue-950/90 to-slate-900/90';
               const borderClass = isHovered 
                 ? (isAttack ? 'border-red-400 shadow-[0_0_30px_rgba(239,68,68,0.6)]' : 'border-blue-400 shadow-[0_0_30px_rgba(59,130,246,0.6)]') 
                 : (isAttack ? 'border-red-900/50' : 'border-blue-900/50');
               ...
               return (
                 ...
                 >
                   ...
                   >
                     ...
                     <div className="flex-1 flex items-center justify-center text-[10px] sm:text-[11px] md:text-sm text-slate-300 text-center leading-snug drop-shadow-md relative z-10">
                       {renderTextWithIcons(tc.desc)}
                     </div>
        ...
        <div className="absolute inset-0 pointer-events-none z-[120] overflow-hidden">
          {rewardEffects.map(effect => (
            <div key={effect.id} className="absolute anim-float-up-fade flex items-center gap-2 text-2xl md:text-3xl font-extrabold drop-shadow-[0_4px_4px_rgba(0,0,0,0.9)] whitespace-nowrap"
               ...
               {renderRewardIcon(effect.type)} {effect.text}
            </div>
          ))}
        </div>
